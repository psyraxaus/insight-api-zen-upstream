...
BlockController.prototype.transformBlock = function(block, info) {
  ...
  if (blockObj.header.version == 3) {
    var certificateIds = blockObj.sidechainCertificates.map(function (cert) {
      return cert.hash;
    })
    obj.cert = certificateIds;
    obj.scTxsCommitment = blockObj.header.scTxsCommitment;
  }
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.bitcoind.getRawBlock(hash, function(err, blockBuffer) {
      ...
      // [MO] I don't see any problems in change this
      var block = bitcore.Block.fromBufferReader(br);

      var info = {};
      info.transactions = [block.transactions[0]];
      ...
      self.node.services.bitcoind.getBlockHeader(hash, function(err, blockHeader) {
        ...
        var summary = {
          ...
          certlength: block.sidechainCertificates.length,
...
BlockController.prototype.getPoolInfo = function(block) {
  if (block.transactions[0].inputs[0]) {
    var self = this;
    var address = block.transactions[0].outputs[0].script.toAddress(this.node.network);
    var bitAddress = address.toString();
    var coinbaseBuffer = block.transactions[0].inputs[0]._scriptBuffer;
    for(var k in this.poolStrings) {
      var kBuffer = Buffer.from(k, "hex");
      if (bitAddress.toString('utf-8').match(k)) {
        return this.poolStrings[k];
      } else if (coinbaseBuffer.toString('utf-8').match(k)) {
        return this.poolStrings[k];
      }
    }
  }
