...
TxController.prototype.transaction = function(req, res, next) {
  ...
  this.node.getDetailedTransaction(txid, function(err, transaction) {
    ...
    self.transformTransaction(transaction, showImmatureBTs, function(err, transformedTransaction) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
...
TxController.prototype.transformTransaction = function(transaction, showImmatureBTs, options, callback) {
  ...
  transformed.vout = transaction.outputs.map(this.transformOutput.bind(this, options, transaction));
  ...
  if(transformed.version == -4) {
    transformed.vsc_ccout = transaction.vsc_ccout;
    transformed.vft_ccout = transaction.vft_ccout;
    transformed.vmbtr_out = transaction.vmbtr_out;
    transformed.vcsw_ccin = transaction.vcsw_ccin;
  }

  if(transformed.version == -5) {
    transformed.cert = transaction.cert;
    transformed.maturityHeight = transaction.maturityHeight;
    transformed.blocksToMaturity = transaction.blocksToMaturity;
    transformed.certificateState = transaction.certificateState;

    if ((transformed.certificateState === "SUPERSEDED") || //Never show superseded BTs
        (showImmatureBTs == 0 && transformed.certificateState === "IMMATURE" ) || //Don't show immature BTs if showImmatureBTs flag is 0
        (transformed.certificateState === "LOW_QUALITY_MEMPOOL") || //Dont show unconfirmed certificate that are low quality
        (showImmatureBTs == 0 && transformed.certificateState === "TOP_QUALITY_MEMPOOL")){ //Don't show the top quality unconfirmed BTs if showImmatureBTs flag is 0
      var tmp = transformed.vout.filter(out =>{return !out.hasOwnProperty('backwardTransfer')});
      transformed.vout = tmp;
    }
  }
...
TxController.prototype.transformOutput = function(options, transactions, output, index) {
  var transformed = {
  ...
  if (output.backwardTransfer) {
    transformed.backwardTransfer = true;
    transformed.maturityHeight = transactions.maturityHeight;
    transformed.blocksToMaturity = transactions.blocksToMaturity;
    if (transactions.certificateState === "MATURE") {
      transformed.mature = true;
    } else {
      transformed.mature = false;
    }
  }
...
TxController.prototype.list = function(req, res) {
  ...
  if(blockHash) {
    self.node.getBlockOverview(blockHash, function(err, block) {
      ...
      var totalTxs = block.version === 3 ? block.txids.length + block.certs.length : block.txids.length;
      var totalTxids = block.version === 3 ? block.txids.concat(block.certs) : block.txids;
      var txids;

      if(!_.isUndefined(page)) {
        var start = page * pageLength;
        txids = totalTxids.slice(start, start + pageLength);
      ...
      async.mapSeries(txids, function(txid, next) {
        self.node.getDetailedTransaction(txid, function(err, transaction) {
          ...
          self.transformTransaction(transaction, showImmatureBTs, next);
        });
    ...
    self.node.getAddressHistory(address, options, function(err, result) {
      ...
      async.map(
        txs,
        function(tx, next) {
          self.transformTransaction(tx, showImmatureBTs, next);
        },
        function(err, transformed) {
          if (err) {
            return self.common.handleErrors(err, res);
